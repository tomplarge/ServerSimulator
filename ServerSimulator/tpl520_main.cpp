    ...
    do {
        ...
        switch(cmd) {
            ...
            case SYSTEM_STATUS: {
                if (number_of_tokens == 1){
                    ...
                    for (int j = 1; j < number_of_tokens; j++){
                        ip.parse(parsed_command[j]);
                        for(i=0; i<MAX_MACHINES; i++) {
                            if(network[i]!=NULL && (network[i]->myAddress()).sameAddress(ip)) {
                                network[i]->display();
                                cout << endl;
                            }
                        }
            ...
            case CONSUME_DATAGRAM: {
                ...
                for (int i = 0; i < MAX_MACHINES; i++){
                    IPAddress ip;
                    ip.parse(parsed_command[1]);
                    if (network[i]->amIThisComputer(ip)){
            ...
            case TIME_CLICK: {
                ...
                for (i = 0; i < MAX_MACHINES; i++)
                {
                    if (network[i] != NULL) {
                        ...
                        if (network[i]->myType() == WAN_MACHINE)
                        {
                            ((WAN*)network[i])->transferDatagram();
                        }
                    }
                }
                break;
